...
public class TopicServiceTest {
	...
	public void testCreateTopicWithMultipleSessionsSuccess() {
		...
		InOrder inOrder = Mockito.inOrder(sessionRepository, topicRepository);
		inOrder.verify(sessionRepository).findById(ids1);
		inOrder.verify(sessionRepository).findById(ids2);
		inOrder.verify(topicRepository).findByNameDescriptionAndDifficulty(name, description, difficulty);
		inOrder.verify(topicRepository, times(1)).save(createdTopic);
